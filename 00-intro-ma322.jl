...
# ╔═╡ c5f46f0f-4584-4922-97cc-69057a353248
begin
	using DifferentialEquations# to solve the related ODE
	using Plots # for plotting
	using PlutoUI # interactivity elements
	using Revise # allows for quicker modification of code
end


# ╔═╡ cd9bae6f-3041-4d62-a41f-805a22cd7b5f
md"""
# Your first dynamical system in Julia

Welcome to your first notebook. Without too much explanation, we'll demonstrate what the Julia with the Pluto.jl notebook can do for you.

!!! warning "Patience needed!"

	The first time you run this notebook, it may take several minutes (maybe up to 10) to fully load (to produce the figure below). This is because Pluto.jl automatically installs all packages it needs if you don't have them. On the second load (and subsequent loads) this should reduce substantially.

>If you haven't yet, please follow the instructions on [MIT's course about Julia](https://computationalthinking.mit.edu/Spring21/installation/) to install Julia on your laptop. It may be possible to use something like [Cocalc](https://cocalc.com) to avoid installing on your laptop, but I haven't tried it out.

"""

# ╔═╡ e0084f61-2229-4f54-8b49-633f29c70b54


# ╔═╡ 5e16ddfe-6d68-461e-92e2-6eb9dc46b99d
md"""
First, let's load the basic packages for visualization and modeling of dynamical systems.

In this Pluto.jl online interface, the order of commands more-or-less does not matter - the code worries about consistency of your commands.
But it's nice to be disciplined and start with listing Julia packages at the top.
"""

# ╔═╡ 33fbfcb1-dfd2-4c83-aab3-39d44486d7ba
md"""
(By the way, notice the eye icon that appears just to the left of the text block when you hover your pointer above it - clicking on it uncovers the Julia code used to generate any part of this notebook. Press "F1" to see the list of keyboard shortcuts that avoids clicking on the eye. And click on the "Live doc" in the lower right corner to give you a running help about commands used --- super useful for first time users.)

"""

# ╔═╡ 0fd329a9-92c5-4490-a0a6-a007ed7f3c40
md"""
## Specifying the dynamical system (model)

The code below implements the so-callel SIR compartmental model of an epidemic in a closed environment.

You can read more about the SIR model in our textbook or (if you're impatient) in [its Wikipedia page]( https://en.wikipedia.org/wiki/Compartmental_models_in_epidemiology#The_SIR_model_without_vital_dynamics)
but for now we'll just take it at face value.

The variables (remember:those vary in time) stand for the number of **S**usceptible (those who can get infected), **I**nfected (those who carry the virus), and **R**emoved (those who are immune, diseased, or otherwise stopped participating in the epidemic.
"""

# ╔═╡ c80816cf-05fe-4c05-bd01-0b66084252f3
begin

    function SIRfun!( du, u, p, t)
		S,I,R = u
		β,γ,N = p
...
# ╔═╡ 22cb8289-cf2a-46e8-9a65-67399252ed0e
md"""
>Notice that we enclose all computational cells in begin/end blocks. In this way it is easier to control what gets output on the screen (only the result of the last line) and it's easier for the Pluto notebook to resolve dependencies between the code. So it's just a good practice until you have a compelling reason to do otherwise. The last line of every begin/end block in Julia is the line whose output is displayed in Pluto notebooks.Since the last line of the code defines the `ODESystem` object that contains our model, its output is printed nicely in $\LaTeX$ without our ever having to touch LaTeX code. Nifty!
"""

# ╔═╡ 9b597022-4ab6-461a-a374-90552e11754e
md"""

The parameters (remember, those are the constants in the model) are 

  - N (population size), 
  - β (the proportion of contacts that results in a successful transmission) and 
  - γ (the rate of removal/recovery/death of the infected).
"""

# ╔═╡ 4fc33a41-4c00-47aa-af8d-1cad54e57b5c
md"""
## Simulating the model

Now it's time to run an example scenario of the progress of the epidemic.

We need to specify the initial number of S,I,R and the values of parameters before we solve the differential equation.

We could set this up using the usual programmatic assignment (think `I0=5` and so on), but let's just jump in and use `Pluto.jl`'s interactivity.

  - Initial number of infected ``I_0`` $(@bind I0 PlutoUI.Slider(0:1:1000;default=5, show_value=true))

  - Transmission rate ``\beta`` $(@bind bval PlutoUI.Slider(0:0.05:1;default=0.1, show_value=true))

  - Duration of recovery ``1/\gamma`` $(@bind igval PlutoUI.Slider(0:5:100;default=20,show_value=true))

  - Total time ``t`` $(@bind tval PlutoUI.Slider(50:10:1000;default=200, show_value=true))

>Pluto.jl allows me to immediately present you the output image of our simulation. To read how `SIR_graph` variable is created, you're going to have to scroll down.
"""

# ╔═╡ 214249f4-9065-4930-9fa8-03392110ce4c
begin
	# parameter values
	
	p = [bval, 1.0/igval, 1000]
	
#  Dictionaries `Dict( ... )` are kind of like arrays, but instead of using integer
# index, they can use anything - in this case, the names of our symbolic parameters.

	# initial conditions
	ic = [p[3] - I0,I0, 0]
end; # semicolon prevents the block from outputting anything



# ╔═╡ a4548f92-a4bb-4687-a3d5-bc0eb6dc999a
md"""
>Values `bval`, `igval`, and `I0` are *bound* to sliders above. Uncover the code specifying the sliders to see how this is done. This is an excellent tool for quickly modifying key values. I suggest you use it liberally in your own code in this class.
"""

# ╔═╡ ca0bfa1f-5fd4-406d-9f5c-2b0424eefffe
md"""
OK, so now we're ready to create an Initial Value Problem (IVP - remember MA232?) and solve it using the computer.

The code below uses our specified model to generate an ODE problem that can be solved by an ODE solver. Perhaps it is obvious, but one model can be used to generate multiple types of problems --- think of the "problem" as a question that you use to interrogate the "model".
"""

# ╔═╡ 17189cfc-098b-4adf-9763-5367d7a44b9d
# let's define the Initial Value Problem	
prob = ODEProblem( 
	SIRfun!, # this variable stores our equations
	ic, 	# this is the initial condition
	[0.0,tval], # this is the time duration 0 -> tval (specified by the slider)
	p # parameter dictionary
);


# ╔═╡ d8e50236-2ef0-45ba-92f6-4dcac691b8b8
md"""
Now, we're going to use a numerical solver to solve the IVP problem above.
"""
...
md"""
More options are available in [DifferentialEquations.jl documentation](https://diffeq.sciml.ai/stable/basics/common_solver_opts/#Output-Control)

>If you want to know more about some function quickly (kind of like `help plot` in MATLAB) you can create a new cell with `@doc functioname` in it. Try it out by running: `@doc DifferentialEquations.ODEProblem`
"""

# ╔═╡ 9740f81c-5bf9-4e3e-bc45-bda153628545
md"""
...
begin
	SIR_graph = plot(sol; vars=(0,1), label="S")
	plot!(sol; vars=(0,2), label="I")
	plot!(sol; vars=(0,3), label="R")	
end;
# again - we put semicolon here so that we suppress any output we don't want, 
# and manually invoke the figure to be reproduced below.



# ╔═╡ 29518598-33ae-4dc2-9592-3caaf3375075
SIR_graph
...
md"""
Finally, we'll add annotations. Here we use the `SIR_axis` variable stored above.
"""

# ╔═╡ e8aa895e-c340-46cd-8c45-d7ebd640ddac
begin
...
# ╔═╡ cd837b41-4528-4e07-acdb-63145379de36
md"""
At this point the variable `SIR_graph` contains our full figure. That's why when 
we invoked `SIR_graph` after the text containing sliders, it was able to
...
# ╔═╡ 48b394a8-b60e-47cc-a48b-84a1f9500073
SIR_graph

# ╔═╡ 5c51a85c-9653-4239-80a8-6ecc85ba98b0
md"""
Finally, let's save the figure to a file.
"""

# ╔═╡ 5b776303-ed17-42a3-ab62-54ac89d33b12
savefig(SIR_graph,"SIRgraph.png")

# ╔═╡ 71efc202-d7cf-4023-a4e9-d1901e77d7e6
md"""
## What's next?

We are going to use Julia, ModelingToolkit, DifferentialEquations, Makie, and other packages extensively in our course.

Depending on your familiarity with Julia, MATLAB, Python, etc. you may want to use some of the following resources:

  - [Start here](https://www.juliafordatascience.com/quickstart/)
  - [Julia Cheatsheets](https://www.juliafordatascience.com/cheat-sheets/)
  - [Markdown commands](https://docs.julialang.org/en/v1/stdlib/Markdown/)
  - [MIT Intro to Comp Thinking](https://computationalthinking.mit.edu/Spring21/)
  -  We are going to use Pluto notebooks exclusively in this class . If you ever start writing more involved programs, you'll likely want to work in a more traditional developing environment (kind of like MATLAB GUI, Eclipse, or Visual Studio). [Here's a guide to VSCode that seems to be preferred by julian community.](https://code.visualstudio.com/docs/languages/julia)
  - [Julia Discourse](https://discourse.julialang.org) - help forum with a dedicated newbie section. I strongly suggest you make an account and ask questions when stuck.
"""

# ╔═╡ 4f8a36e9-cd6a-4f77-8d30-4a775abae23f
md"""
## P.S. Why are you making me use Julia?!

Alright, it's a fair question. There's a pedagogical and practical part to this answer.

You may not care about pedagogical, so let's tackle practical first.

There is no other language out there that has all of the below:
  - low barrier to entry in the form of active notebooks (so we can mostly focus on math),
  - well-integrated symbolic and numerical math engines (so we can do simulations and analysis in the same environment),
  - vibrant community of users (so you can google questions when you get stuck),
  - is general-purpose enough that it's not for math-only (so you can use it even outside my class).
Yes, we could do things in MATLAB, but it would require you to spend more time programming than exploring dynamical systems.
Yes, we could do things in Mathematica, but we would have to get a license, the community is... let's just say "poor" and leave it at that, and in the end, if you get a job in math-adjacenty field, you'd never use it again.

From pedagogical standpoint, I want to help you realize that adapting to different programming languages is part-and-parcel of modern mathematics. At universities, maybe you'll use Mathematica, your first job may require you to use Python, then you'll move and that place will use MATLAB, and then you decide you want to live elsewhere, so you move another time, and it's Python again, but it's evolved since you last touched it so you have to re-learn it again.

Almost nobody today does math *without* computers, and computing environments change so often and so rapidly that we have to accept this as a fact and prepare you for it, rather than insulate you from it.
